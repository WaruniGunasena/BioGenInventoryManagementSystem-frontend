...
import { CourierChargesPopup } from "../../pages/SalesOrder/CourierChargesPopup";
...
import "../../pages/SalesOrder/SalesOrder.css";
...
/**
 * Shared Sales Order form component.
 *
 * Props:
 *  - mode          : "create" | "edit"
 *  - initialInvoice: existing invoice object (required when mode="edit")
 */
const SalesOrderForm = ({ mode = "create", initialInvoice = null }) => {
    ...
    const fetchCustomers = async () => {
        try {
            ...
            else if (res.data?.customers && Array.isArray(res.data.customers)) data = res.data.customers;
            setCustomers(data);
        } catch (error) {
    ...
    const fetchProducts = async () => {
        try {
            const [prodRes, stockRes] = await Promise.all([getAllProducts(), getAllStock()]);

            let prodData = [];
            if (prodRes.data && Array.isArray(prodRes.data)) prodData = prodRes.data;
            else if (prodRes.data?.customers && Array.isArray(prodRes.data.customers)) prodData = prodRes.data.customers;
            else if (prodRes.data?.products && Array.isArray(prodRes.data.products)) prodData = prodRes.data.products;
            setProducts(prodData);

            let sData = [];
            if (stockRes.data && Array.isArray(stockRes.data)) sData = stockRes.data;
            else if (stockRes.data?.productStocks && Array.isArray(stockRes.data.productStocks)) sData = stockRes.data.productStocks;
            setStockData(sData);
        } catch (error) {
    ...
    useEffect(() => {
        if (!isEdit || !initialInvoice || customers.length === 0 || products.length === 0) return;

        const customer = customers.find(
            (c) => (c.id || c._id) === initialInvoice.customerId || c.name === initialInvoice.customerName
        );
        ...
        if (customer) {
            ...
            setFormData((prev) => ({
                ...
                date: initialInvoice.date || prev.date,
            }));
        } else {
            setFormData((prev) => ({
                ...prev,
        ...
        setCourierCharges(initialInvoice.courierCharges || "");
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [initialInvoice, customers, products]);
    ...
    const handleInputChange = (e) => {
        ...
        setFormData((prev) => {
            ...
            if (["sellingPrice", "quantity", "discountPercent", "discountedPrice"].includes(name)) {
                ...
                if (name === "sellingPrice" || name === "discountPercent") {
                ...
                const activePrice =
                    (newData.discountPercent === "" && newData.discountedPrice === "") ||
                    (parseFloat(newData.discountPercent) === 0 &&
                        parseFloat(newData.discountedPrice) === 0 &&
    ...
    const handleCancelEdit = () => {
        ...
        setFormData((prev) => ({ ...prev, ...EMPTY_PRODUCT_FIELDS }));
    };
    ...
    const handleAddItem = () => {
        ...
        if (requestedTotal > availableQty) {
            showToast(
                "error",
                `Only ${availableQty} units of "${formData.productName}" are available in stock. You cannot issue ${requestedTotal} units right now.`
            );
            return;
        }

        const isDuplicate = addedItems.some(
            (item) => item.productId.toString() === formData.productId.toString() && item.id !== editingItemId
        );

        if (isDuplicate) {
            showToast(
                "error",
                `Product "${formData.productName}" is already added. Please edit the existing entry.`
            );
            return;
        }
    ...
    const calculateTotal = () => addedItems.reduce((acc, item) => acc + parseFloat(item.totalAmount), 0);
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isEdit) {
                ...
                const invoiceNumber = res.data?.invoiceNumber || res.data?.data?.invoiceNumber || initialInvoice.invoiceNumber;
                showToast("success", `Sales Order updated successfully! Invoice No: ${invoiceNumber || ""}`);
                setTimeout(() => navigate("/sales-invoices"), 1000);
            } else {
            ...
            showToast("error", error.response?.data?.message || "Failed to submit bill. Please try again.");
        }
    };
    ...
    const availableLimit = selectedCustomerData
        ? (parseFloat(selectedCustomerData.creditLimit) || 0) - (parseFloat(selectedCustomerData.dueBalance) || 0)
        : 0;
    const isOverLimit =
        selectedCustomerData &&
        selectedCustomerData.creditPeriod !== "Cash" &&
        grandTotalValue > availableLimit;

    const pageTitle = isEdit ? "Edit Sales Invoice" : "Sales Invoice";
    const submitLabel = isEdit ? "Save Changes" : "Issue Bill";
                            ...
                            <div className="sales-order-row">
                                ...
                                <div className="sales-order-field">
                                    ...
                                    <input
                                        ...
                                        value={
                                            formData.creditTerm
                                                ? formData.creditTerm === "cash"
                                                    ? "Cash"
                                                    : `${formData.creditTerm} days`
                                                : "Not Specified"
                                        }
                                        readOnly
                                    />
                        ...
                        <div className="sales-order-summary-section">
                            ...
                            <div className="invoice-info-bar">
                                <div className="info-col">
                                    ...
                                    <span className="info-value">{formData.invoiceNumber || "Auto Generated"}</span>
                                </div>
                                <div className="info-col">
                                    ...
                                    <div className="box-content">
                                        {selectedCustomerData ? (
                                            <>
                                                ...
                                                <p>
                                                    <b>Available Limit : </b>
                                                    Rs.{selectedCustomerData.creditLimit - selectedCustomerData.dueBalance}
                                                </p>
                                            </>
                                        ) : (
                            ...
                            <div className="sales-order-table-container">
                                        ...
                                        {addedItems.map((item, index) => (
                                            <React.Fragment key={item.id}>
                                                ...
                                                {parseFloat(item.preIssue || 0) > 0 && (
                                                    <tr
                                                        className="bonus-row"
                                                        style={{ backgroundColor: "#f9f9f9", fontStyle: "italic", opacity: 0.9 }}
                                                    >
                                                        <td></td>
                                                        <td>
                                                            <div className="product-desc-cell">
                                                                <span className="main-desc" style={{ color: "#666" }}>
                                                                    {item.productName} (Free-Issue)
                                                                </span>
                                ...
                                <div className="invoice-totals-section">
                                    ...
                                    <div className="totals-content">
                                        ...
                                        <div className="totals-line grand-due">
                                            <span className="line-label" style={isOverLimit ? { color: "red" } : {}}>
                                                Grand Total (LKR)
                                            </span>
                                            <span
                                                className="line-value"
                                                style={isOverLimit ? { color: "red", fontWeight: "bold" } : {}}
                                            >
                                                {calculateGrandTotal()}
                                            </span>
                                        </div>
                                        {isOverLimit && (
                                            <div
                                                ...
                                                style={{ color: "red", fontSize: "0.80rem", marginTop: "10px", justifyContent: "flex-end", textAlign: "right" }}
                                            >
                                ...
                                <div className="invoice-footer-actions">
                                    <button className="issue-bill-btn" onClick={handleSubmit} disabled={isOverLimit}>
                                        {submitLabel}
                                    </button>
                                </div>
                            </div>
